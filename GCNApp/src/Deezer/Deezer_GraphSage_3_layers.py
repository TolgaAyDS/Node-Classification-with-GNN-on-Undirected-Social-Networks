...
class GraphSAGE(nn.Module):
    def __init__(self, in_feats, hidden_feats, hidden_feats_2, out_feats, num_layers, aggregator_type='mean'):
        super(GraphSAGE, self).__init__()
        self.convs = nn.ModuleList()
        self.convs.append(SAGEConv(in_feats, hidden_feats, aggregator_type))

        for _ in range(num_layers - 2):
            self.convs.append(SAGEConv(hidden_feats, hidden_feats_2, aggregator_type))

        self.convs.append(SAGEConv(hidden_feats_2, out_feats, aggregator_type))
...
def run(epoch_num=100, validation=True):
    ...
    hidden_feats = 500  # Number of hidden units
    hidden_feats_2 = 50  # Number of hidden units
    out_feats = 2  # Number of output classes
    num_layers = 3  # Number of GraphSAGE layers

    # Print hyperparameters
    print(f"Hyperparameters:\n"
          f"Number of input features: {in_feats}\n"
          f"Number of hidden units: {hidden_feats}\n"
          f"Number of hidden units: {hidden_feats_2}\n"
          f"Number of output classes: {out_feats}\n"
          f"Number of GraphSAGE layers: {num_layers}\n")

    # Create the GraphSAGE model
    model = GraphSAGE(in_feats, hidden_feats, hidden_feats_2, out_feats, num_layers)

    # print out the model architecture
    print("Architecture of the GraphSAGE model:")
    print(model)
...
if __name__ == "__main__":
    ...
    run(epoch_num=100, validation=True)
